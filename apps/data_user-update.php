...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    $selectSql = "SELECT id, username FROM users WHERE id = ? LIMIT 1";
    ...
    if ($username_err === '' && $password_err === '' && $confirm_password_err === '') {
        ...
        $username_err = "Gagal memperbarui user.";
        if ($stmt) {
            mysqli_stmt_close($stmt);
        }
    }
...
<head>
    ...
    <title>PT. SUGIH BOGA NUSANTARA - Edit User</title>
                    ...
                    <div class="card-body">
                        <form method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" id="password" name="password" class="form-control <?php echo $password_err !== '' ? 'is-invalid' : ''; ?>">
                                <small class="form-text text-muted">Kosongkan jika tidak ingin mengganti password.</small>
                                <span class="invalid-feedback"><?php echo escape_html($password_err); ?></span>
                            </div>
                            <div class="form-group">
                                <label for="confirm_password">Konfirmasi Password Baru</label>
                                <input type="password" id="confirm_password" name="confirm_password" class="form-control <?php echo $confirm_password_err !== '' ? 'is-invalid' : ''; ?>">
